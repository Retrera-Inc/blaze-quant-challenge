...
const BSC = () => {
  ...
  const dimIntegers = [0.9, 0.5, 0.2];
  const gradientPlanet1 = { clr1: `#0061ff`, clr2: `#0061f0` };
  const gradientPlanet2 = { clr1: `#40c9ff`, clr2: `#0061f0` };
  const gradientPlanet3 = { clr1: `#00ff87`, clr2: `#60efff` };
  const planetStyles = [
    {
  ...
  const planetRadius = [27, 47, 60];
  const planetRadian = [
    Math.PI * 2 * 0.1,
    Math.PI * 2 * 0.9,
    Math.PI * 2 * 0.2,
  ];
  const planetVelocity = [1, 0.5, 0.7];
  const sunStyle = {
  ...
  const containerStyle = { height: `6vw`, width: `6vw` };
  ...
  return (
    <div className="system-container" style={containerStyle}>
      <div className="bsc-sun sun" ref={sun} style={sunStyle}></div>
      <div
        className="bsc-planets planets"
        ref={planet[0]}
